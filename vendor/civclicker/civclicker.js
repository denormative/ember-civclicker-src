...
function getPurchaseCellText(purchaseObj, qty, inTable) {
    ...
    if (allowPurchase())
    {
        s +="<button class='compact ui button x"+abs(qty)+"' data-action='purchase'"+" disabled='disabled'>"+fmtqty(qty)+"</button>";
    }
    s += "</"+tagName+">";
    return s;
}
...
function getPantheonUpgradeRowText(upgradeObj)
{
    ...
    s += "<td class='"+upgradeObj.type+"true'><button id='"+upgradeObj.id+"'  class='compact ui button xtrue'";
...
//window.setInterval(runLoop, 1000); //updates once per second (1000 milliseconds)
